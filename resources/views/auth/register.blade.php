    ...
    <div class="relative z-10 min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0">
        ...
        <div class="w-full sm:max-w-md px-8 py-10 bg-white/10 backdrop-blur-xl border border-white/20 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] overflow-hidden sm:rounded-[2rem]">
            ...
            <div class="mb-8 text-center">
                <h2 class="text-2xl font-black text-white tracking-widest uppercase italic">Create Account</h2>
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                {{-- Name Field --}}
                <div class="group">
                    <label for="name" class="block font-black text-[10px] text-indigo-200 uppercase tracking-widest ml-1 mb-1">{{ __('Full Name') }}</label>
                    <x-text-input id="name" 
                        ...
                        type="text" name="name" :value="old('name')" required autofocus placeholder="Enter Name" />
                    <x-input-error :messages="$errors->get('name')" class="mt-2 text-xs text-rose-400 font-bold" />
                </div>

                {{-- Email Field --}}
                <div class="mt-4 group">
                    <label for="email" class="block font-black text-[10px] text-indigo-200 uppercase tracking-widest ml-1 mb-1">{{ __('Email') }}</label>
                    <x-text-input id="email" 
                        class="block w-full bg-white/5 border-white/10 text-white placeholder-white/20 focus:ring-indigo-500/50 focus:border-indigo-500 focus:bg-white/10 transition-all duration-300 rounded-xl" 
                        type="email" name="email" :value="old('email')" required placeholder="Enter Email" />
                    <x-input-error :messages="$errors->get('email')" class="mt-2 text-xs text-rose-400 font-bold" />
                </div>

                {{-- Password Field with Eye Toggle --}}
                <div class="mt-4 group" x-data="{ show: false }">
                ...
                <div class="mt-4 group" x-data="{ show: false }">
                    <label for="password_confirmation" class="block font-black text-[10px] text-indigo-200 uppercase tracking-widest ml-1 mb-1">{{ __('Confirm Password') }}</label>
                    <div class="relative">
                    ...
                    <x-input-error :messages="$errors->get('password_confirmation')" class="mt-2 text-xs text-rose-400 font-bold" />
                </div>
                ...
                <div class="mt-10">
                    ...
                    <div class="mt-8 pt-6 border-t border-white/10 text-center">
                        <a class="text-[10px] font-bold text-white/40 hover:text-indigo-300 transition uppercase tracking-tighter" href="{{ route('login') }}">
